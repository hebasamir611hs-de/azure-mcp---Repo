...
6. **Automate the test case** — translate each `Automation`-tagged case into a pytest
   ...
   subset, plus the Platform marker `ios` / `android`), assertions in the test (not the
   Screen Object). A `Manual`-tagged case is never automated; real hardware steps (e.g.
   tag at the gun) are exactly what gets tagged `Manual`.
